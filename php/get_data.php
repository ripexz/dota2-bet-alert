<?php
	...
	while ( page_found( $d2url . $next ) ){
		...
		$sql = mysqli_query($db, "INSERT INTO match_data (id, status, t1, t2, chance1, chance2) VALUES ($next, '{$status}', '{$team1}', '{$team2}', '{$chance1}', '{$chance2}')");
		$next++;
	}
	...
	if ( $result2 && mysqli_num_rows($result2) > 0 ) {
		while ( $row = mysqli_fetch_assoc($result2) ) {
			...
			$sql2 = mysqli_query($db, "UPDATE match_data SET status = '{$status}', t1 = '{$team1}', t2 = '{$team2}', chance1 = '{$chance1}', chance2 = '{$chance2}' WHERE id = {$curr_id}");
		}
	}
